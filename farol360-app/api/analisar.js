...
const MODELOS = {
  ...
  sonnet: { id: 'claude-sonnet-5',   max: 6500 },
  opus:   { id: 'claude-opus-4-8',   max: 8500 },
};
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'assistant', content: '{' }, // prefill: força a saída a começar em JSON
        ],
      }),
    });
    ...
    if (!r.ok) {
      ...
      res.status(502).json({ erro: 'A IA recusou a chamada.', detalhe: t.slice(0, 500) });
      return;
    }
    ...
    catch (e) { res.status(502).json({ erro: 'A IA não devolveu JSON válido.', bruto: texto.slice(0, 800) }); return; }
